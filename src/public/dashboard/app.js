...
const manualLocationPopover = document.getElementById('manual-location-popover');
...
document.addEventListener('click', (e) => {
    if (!manualLocationPopover.hidden &&
        !manualLocationPopover.contains(e.target) &&
        !manualLocationBtn.contains(e.target)) {
...
async function fetchWeather(lat, lon, reqHeaders) {
...
function getConditionIcon(summary) {
...
function getScoreCaption(tone, alerts) {
    if (alerts)
        return 'Adverse weather occurring or expected. Best to stay indoors';
    switch (tone) {
        case 'good':
...
function formatAlertTime(unixSeconds) {
...
`;
    ...
    const listItems = userForecast.next3HoursSummary.map((forecast) => {
        ...
        li.innerHTML = `
            <span class="material-symbols-rounded">${getConditionIcon(forecast.icon)}</span>
    ...
    if (userForecast.alerts)
        renderWeatherAlerts(userForecast.alerts);
...
function getCalorieTone(consumed, goal) {
    return goal > 0 && consumed > goal ? 'poor' : 'calorie';
}
function renderTodaysCalories(todaysCalories) {
...
function buildCalorieField(config) {
    ...
    const input = editor.querySelector('.calorie-editor-input');
    function open() {
...
async function bootstrapWeatherData(lat, lon) {
    const userForecast = await fetchWeather(lat, lon);
    return renderWeather(userForecast);
}
async function bootstrapCaloriesData() {
    const todaysCalories = (await fetchTodaysCalories());
    if ('statusCode' in todaysCalories) {
        ...
        renderTodaysCalories(todaysCalories);
}
