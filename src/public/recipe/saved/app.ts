...
async function fetchSavedRecipes() {
    try {
        ...
        const userRecipes: UserRecipes = await res.json();
...
recipeGrid!.addEventListener('click', async (e) => {
    ...
    const card = e.target.closest('.recipe-card') as HTMLLIElement;
    if (!card) return;
...
function renderSavedRecipes(userRecipes: UserRecipes) {
    ...
    if (userRecipes.length === 0) return (document.getElementById('empty-state')!.hidden = false);
