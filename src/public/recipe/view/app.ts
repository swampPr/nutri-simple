import { AccessTokenResponse } from '../../common/types/accessToken.js';
import { RecipeInstructionsResponse } from '../../common/types/recipeInstructions.js';
...
const backLink = document.getElementById('back_link') as HTMLAnchorElement;
const recipeImage = document.querySelector('[data-recipe-image]') as HTMLImageElement;
const sourceLink = document.querySelector('[data-recipe-source]') as HTMLAnchorElement;
const recipeId = parseInt(searchParams.get('id')!);

const previousUrl = new URL(document.referrer).pathname;

async function fetchRecipeInstructions(recipeId: number) {
    try {
        ...
        if (!res.ok) throw recipeInstructions;
...
function renderRecipeInstructions(recipeInstructions: RecipeInstructionsResponse) {
    const ingredientsList = document.querySelector('[data-ingredient-list]') as HTMLUListElement;
    ...
    const macroItems = recipeInstructions.recipeInfo.nutrition.nutrients.filter((nutrient) =>
        macros.includes(nutrient.name)
    );
    const nutrientItems = recipeInstructions.recipeInfo.nutrition.nutrients.filter(
        (nutrient) => !macros.includes(nutrient.name)
    );
    ...
    const macroDailyValue: Record<string, number> = {
        Carbohydrates: 275,
        Protein: 50,
        Fat: 78,
    };
    ...
    const uniqueEquipments = equipmentArr.filter(
        (equipment, idx) => equipmentArr.findIndex((e) => e.name === equipment.name) === idx
    );
...
async function tokenRefresh() {
    try {
        ...
        const tokenResponse = (await res.json()) as AccessTokenResponse;
        if (res.status === 401) return (window.location.href = '/login');
        if (!res.ok) throw tokenResponse;
